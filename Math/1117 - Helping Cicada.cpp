...
ll Range_Divisible_by_a(ll l , ll r, ll a) {
    ll divs = (r / a) - ((l - 1LL) / a);
    return divs;
}
...
int main() {
    ...
    Test {
        ...
        for (int i = 0; i < v.size(); i++) {
            ll lc = 1;
            for (ll x : v[i]) lc = Lcm(lc , x);
            if (v[i].size() == 0) continue;
            if (v[i].size() % 2 == 1) divisible += Range_Divisible_by_a(1 , n , lc);
            else divisible -= Range_Divisible_by_a(1 , n , lc);
        }
        ll ans = n - divisible;
        cout << "Case " << tc << ": " << ans << "\n";
    }
...
divisible_by_a = m / a - (n - 1) / a
divisible_by_b = m / b - (n - 1) / b
divisible_by_a_and_b = m / lcm(a , b) - (n - 1) / lcm(a , b) 

divisible_by_a_or_b = a + b - divisible_by_a_and_b


not_divisible_by_a_or_b = total_number - divisible_by_a_or_b
*/
