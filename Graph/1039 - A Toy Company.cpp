...
#define FasterIO     cin.tie(0);cout.tie(0); 
...
bool IsForbidden(string &s) {
    for (auto it : v) {
        ...
        for (int i = 0; i < it.second.first.size(); i++) {
            if (s[1] == it.second.first[i]) {
                cnt++;
                break;
            }
        }
        for (int i = 0; i < it.second.second.size(); i++) {
            if (s[2] == it.second.second[i]) {
                cnt++;
                break;
            }
        }
...
int main() {
    ...
    Test {
        ...
        if (IsForbidden(a) || IsForbidden(b)) {
            cout << "Case " << tc << ": -1\n";
            continue;
        }
        queue <string> q;
        ...
        while (!q.empty()) {
            ...
            if (fr == b) break;
            for (int i = 0; i < 3; i++) {
                string next = fr , prev = fr;
                next[i] = NextChar(next[i]);
                prev[i] = PrevChar(prev[i]);
                if (mp[next] == 0 && !IsForbidden(next)) {
                    mp[next] = mp[fr] + 1;
                    q.push(next);
                }
                if (mp[prev] == 0 && !IsForbidden(prev)) {
                    mp[prev] = mp[fr] + 1;
                    q.push(prev);
                }
            }
