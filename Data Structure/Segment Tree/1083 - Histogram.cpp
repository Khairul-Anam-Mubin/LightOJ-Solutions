...
const int mxN = 30000;
vector <pair <int ,int>> seg(4 * mxN , {INF9 , INF9}); 
void Build(vector <int> &v, int cur, int left , int right) {
    if (left == right) {
        seg[cur] = {v[left] , left};
        return;
    }
    int mid = (left + right) >> 1;
    Build(v , cur << 1 , left , mid);
    Build(v , cur << 1 | 1 , mid + 1 , right);
    seg[cur] = min(seg[cur << 1] , seg[cur << 1 | 1]);
}
pair <int , int> Query(int cur , int left , int right , int l , int r) {
    if (r < left || l > right) return {INF9, INF9};
    if (l <= left && r >= right) return seg[cur];
    int mid = (left + right) >> 1;
    pair <int , int> p = Query(cur << 1 , left , mid, l , r);
    pair <int , int> q = Query(cur << 1 | 1, mid + 1, right , l , r);
    return min(p , q);
} 
ll GetArea(int left , int right, int n) {
    if (right < left) return 0;
    pair <int , int> p = Query(1 , 0 , n - 1, left , right);
    ll now = p.first * (right - left + 1);
    ll ans = max(GetArea(left , p.second - 1, n) , GetArea(p.second + 1 , right, n));
    return max(now , ans);
}
int main() {
    FasterIO
    Test {
        ...
        Build(v , 1 , 0 , n - 1);
        ll ans = GetArea(0 , n - 1, n);
        cout << "Case " << tc << ": ";
        cout << ans << "\n";
        seg.assign(4 * mxN, {INF9 , INF9});
    }
    return 0;
}
